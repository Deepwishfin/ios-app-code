...
+(id)layer{
    ...
    layer.outlineColor = [UIColor colorWithWhite:130/255. alpha:0.7];
    return layer;
}
...
-(void)drawInContext:(CGContextRef)ctx{
    ...
    CGFloat cirRad =  MIN(CGRectGetWidth(self.bounds), CGRectGetHeight(self.bounds))/2-1;

 //   CGFloat angleStep = 2*M_PI/(_numSegments+1);
//    CGFloat startAngle = angleStep/2 + M_PI_2;
    CGFloat startAngle = 3;
    _circleradius = cirRad;
    [self.outlineColor setStroke];
    
    UIBezierPath *circlePath = [UIBezierPath bezierPathWithArcCenter:centerPoint radius:_circleradius startAngle:startAngle endAngle:6.38 clockwise:YES];
    [circlePath stroke];
    
    circlePath = [UIBezierPath bezierPathWithArcCenter:centerPoint radius:_circleradius-_circlewidth startAngle:startAngle endAngle:6.38 clockwise:YES];
    [circlePath stroke];
