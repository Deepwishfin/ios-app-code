...
class ActiveDealsVC: UIViewController {
    
    @IBOutlet weak var tblView: UITableView!
    var cibilId = ""
    var activeDealsArry = [activeDealsModel]()
...
extension ActiveDealsVC: UITableViewDataSource, UITableViewDelegate {
...
//    }
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        cell.dealsDataShow(dict: activeDealsArry[indexPath.row], viewContr: self, indexPath: indexPath.row)
        
        cell.expire_Soonlbl.isHidden = true
        if self.compareDateexpireBool(dStr: activeDealsArry[indexPath.row].expire_date) {
//            cell.expire_dateLbl.text = "Expire Soon"// activeDealsArry[indexPath.row].expire_date
//            cell.expire_dateLbl.textColor = .red
            
            cell.expire_Soonlbl.isHidden = false
        }
        
        cell.expire_TitleLbl.text = "Expires:"
    ...
    func compareDateexpireBool(dStr:String) -> Bool {
         
        let soon = Date().addingTimeInterval(15*24*60*60)
         
        let dateFormatter = DateFormatter()
        dateFormatter.dateFormat = "yyyy'-'MM'-'dd'"
        guard let sdate = dateFormatter.date(from: dStr) else { return false }
        
        let myString = dateFormatter.string(from: soon)
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        vc!.detalsArry = [activeDealsArry[indexPath.row]]
...
//MARK: Get Api call Active Deals
extension ActiveDealsVC {
    func activeDealsApi() {
        if NetworkManager.sharedInstance.isInternetAvailable(){
            self.showHUD(progressLabel: "Loading...")
            guard let cibilId = UserDefaults.standard.value(forKey: DefaultsKey.cibilId) else { return }
            let firstUrl = UrlName.baseUrl + UrlName.activeDeals
                    ...
                    if let resultDic = jsonValue["result"]?.arrayValue {
                        ...
                        for items in resultDic.reversed() {
                            ...
                            if self.compareDateForBool(dStr: daStr) {
                                self.activeDealsArry.append(activeDealsModel(json: items))
                            }
                        }
                        print(resultDic.count)
                        NotificationCenter.default.post(name: Notification.Name("dataSetOnDeals"), object: nil, userInfo: ["activeDeal":"\(self.activeDealsArry.count)"])
